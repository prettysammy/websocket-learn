 ...
 * create a text frame
 * @para {string} data
 * @para {boolean} [masked=false] if the frame should be masked
 * @returns {Buffer}
 * @private
 */
...
* @param {boolean} [fisst=true] if this is the first frame in a sequence
 ...
 * @param {number} code
 * @param {string} [reason='']
 * @param {boolean} [masked=false] if the frame shoule be masked
 * @returns {Buffer}
 * @private
 */
...
/**
 * create the meta-data portion of the frame
 * @param {boolean} fin
 * @param {number} opcode
 ...
 * @private
 */
exports.generateMetaData = function(fin, opcode, masked, payload){
	var len,meta,start,mask,i;

	len = payload.length;

	//create the buffer for meta-data
	meta = new Buffer(2+ (len < 126 ? 0 : (len < 65536 ? 2 : 8)) + (masked ? 4 : 0));

	//set din and opcode
	meta[0] = (fin ? 128 : 0) + opcode;

	//set the mask and length
	meta[1] = masked ? 128 : 0;
	start = 2;
	if(len < 126){
		meta[1] += len;
		...
		//warn: JS dosen`t support intergers greater than 65536
		meta[1] += 127;
